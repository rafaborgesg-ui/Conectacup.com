...
// Lista de status que são considerados padrão do sistema
const SYSTEM_DEFAULT_STATUS = [
  'Disponível',
  'Em Uso',
  'Descartado',
  'Reservado'
...
export function StatusRegistration() {
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      // Impede deletar status padrão do sistema
      if (statusToDelete && SYSTEM_DEFAULT_STATUS.includes(statusToDelete.name)) {
  ...
  const defaultStatus = safeStatusList.filter(s => SYSTEM_DEFAULT_STATUS.includes(s.name)); // Status padrão do sistema
  const customStatus = safeStatusList.filter(s => !SYSTEM_DEFAULT_STATUS.includes(s.name)); // Resto são personalizados
    ...
    <div className="flex-1 px-2 py-2 sm:p-4 lg:p-8 w-full max-w-full">
      <div className="max-w-4xl lg:mx-auto w-full">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Input
                  ...
                  maxLength={50}
                  disabled={editingId ? SYSTEM_DEFAULT_STATUS.includes(statusList.find(s => s.id === editingId)?.name || '') : false}
                />
                {editingId && SYSTEM_DEFAULT_STATUS.includes(statusList.find(s => s.id === editingId)?.name || '') && (
        ...
        <div className="space-y-6">
          {/* Informação sobre Status Padrão */}
          {defaultStatus.length > 0 && (
            <Card className="p-4 bg-blue-50 border border-blue-200">
              <div className="flex items-start gap-3">
                <Shield className="text-blue-600 mt-0.5 flex-shrink-0" size={20} />
                <div className="flex-1">
                  <h4 className="text-blue-900 mb-1">Sobre os Status Padrão</h4>
                  <p className="text-sm text-blue-700">
                    Os status <strong>Disponível, Em Uso, Descartado e Reservado</strong> são essenciais para o funcionamento do sistema. 
                    Você pode editar suas cores e ordem de exibição, mas não pode renomeá-los ou excluí-los.
                  </p>
                </div>
              </div>
            </Card>
